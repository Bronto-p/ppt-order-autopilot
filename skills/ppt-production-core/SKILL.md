...
2. worker 只返回 render result；parent agent 对 exact content、素材、文字、风格和页面结构做 QA。
3. 通过则记录 `accepted_attempt`，把选中图复制到 canonical `origin_image/slide_XX.png`。
4. 未通过则保存失败分类和证据，创建窄范围 repair job，再派一个单页 repair worker。
5. 缺真实素材或内容冲突立即 blocked；backend 暂时失败、文字错误、style drift 或 layout error 才能自动重试。
6. 不覆盖历史 attempt，不超过 base job 的 `max_attempts`，超过后进入 owner review。
