...
-> OWNER_SAMPLE_PRODUCTION -> OWNER_SAMPLE_REVIEW
...
- 正式逐页生产前必须先生成一张带真实内容的完整页面样稿，在 Codex 中发给 owner 审核并停止。背景图、moodboard、style anchor 或空白模板不算样稿。
- owner 批准后绑定样稿 manifest hash，再生成 style kit 和全稿；修改意见只重做样稿。“完成后直接返回”不等于授权跳过样稿。
...
样稿是唯一个默认 owner-direct 设计确认点：展示完整样稿页并等待批准。其他 owner gate 只用于客户承诺或真实高影响不确定性：接单、价格、截止、范围、客户消息、最终外发、重大改稿、关键素材或事实冲突。
