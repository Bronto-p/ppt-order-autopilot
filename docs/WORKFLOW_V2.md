...
这个流程不假设客户一定有模板、一定有内容、一定要样稿，也不假设所有页面都适合用同一种方式生成。
...
| output mode | image-first / hybrid / template-native / editable reconstruction | 决定页面如何生成和装配 |
| sample policy | required / risk-based / explicitly skipped | 决定是否先做视觉确认 |
...
| 有模板和完整内容 | 模板解析 → 页型映射 → template-native 或 hybrid | 模板字体/母版缺失，内容超出页型容量 |
...
| 需要完全可编辑 | template-native 或 editable reconstruction | 客户只提供低清截图且要求像素级可编辑 |
| 高视觉路演/发布会 | image-first 页面 + native text safety layer | 品牌素材缺失、人物/产品不可伪造 |
...
worker 输出：页面图或视觉层、可编辑层描述、实际使用素材、内容核对结果、阻塞项和可重试原因。
...
适合封面、章节页、概念页、情绪页和低文字高视觉页面。生成完整视觉画面，必要时用 native text safety layer 覆盖关键标题和数字。

### hybrid

适合大多数商业内容页。图片模型生成背景、构图或插图；标题、正文、数字、表格和图表使用可校验的原生元素。它兼顾视觉质量、文字准确性和一定可编辑性。
