...
| output mode | image_first / hybrid / template_native / editable_reconstruction | 决定页面如何生成和装配 |
...
`owner_direct` 是同一生产核心的短路径：直接文件 intake → brief/contract → internal style anchor → per-slide production → QA → verified Codex return。它不经过客户报价、样稿发送或外部交付 gate；最终由 `autopilot.py finish --target owner` 生成 receipt。
