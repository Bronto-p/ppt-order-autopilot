...
11. 在 owner 已批准且 file-send manifest 完整时，发送样稿、成品或改稿文件。
...
4. 不允许自行报价。
5. 不允许承诺截止时间。
6. 不允许发送未经 `send_sample`、`send_final_delivery` 或 `send_revision_response` 审批的文件。
7. 不允许删除、撤回、转发聊天。
8. 不允许修改企业微信设置。
...
文件发送还必须重新计算 manifest 内所有本地文件 hash，确认附件文件名与数量，并在发送前后保存截图。审批后文件内容发生变化时，旧 approval 自动失效。
